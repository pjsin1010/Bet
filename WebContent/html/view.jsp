...
<%@ page import ="com.ryan.dao.BoardListDAO" %>
<%@ page import ="com.ryan.dao.BoardViewDAO" %>
<%@ page import ="com.ryan.dto.viewDTO" %>
<%@ page import ="com.ryan.dto.*" %>
<%@ page import ="com.ryan.dao.CommendListDAO" %>
...
<%
 ...
 viewDTO dto = BoardViewDAO.boardView(n);
...
#inputContent:focus {  color: #00FF00; border: solid 1px #00FF00; outline: none;} 
...
<tr>
		...
		function Edit(){
			...
			<img src="./images/btnModify.png" onclick="Edit()"; style="cursor:pointer">
...
<form  method="post" action="./commendinsert.ap?num=<%=dto.getNum() %>">
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="commentForm"  style="margin-top:25px;"  align="center">
	<tbody align="center" ><tr>
		...
		<td width="794" class="conntent"><textarea name="content" id="inputContent" name="content" rows="3" class="text2" style="width:98%;"></textarea></td>
...
<%
	...
	// 전체 갯수
	int cnt =CommendListDAO.getCount(dto.getNum());
	...
	ArrayList<CommendDTO> list = CommendListDAO.commendinfo(dto.getNum() , start , exit);
...
<% for ( CommendDTO m : list){ %>
<tbody><tr>
	<td width="159" valign="center" class="write"  >&nbsp;&nbsp;<img src="./images/level/<%=m.getLevel() %>.gif" border="0" align="absmiddle"> <%=m.getName() %></td>
	<td width="646" class="content" valign="top" align="left"><font color="#ffffff"> <%=m.getContent() %></font><br><br></td>
	<td width="161" valign="center" class="regdate"><font color="#ffffff"><%=m.getIndate() %></font>&nbsp;&nbsp;
	...
	<a href="./commentdel.ap?num=<%=dto.getNum()%>&num1=<%=m.getNum()%>">
	
	<img src="./images/pop_close.png" border="0" onclick="Del1();" style="cursor:pointer; vertical-align:middle;"></td></a>
<%  } %>
	<script>
<!--
	function Del1(){
			location.href="./php/commentdel.php?num=<?php echo $row[num]?> ";
	}
//-->
</script>
</tr>
...
<nav>
    ...
    <%for (int i=1; i<=totalpage; i++) { %>
 	<% if( i != ipage + 1){ %>
 	<li class="page-item"><a class="page-link" href="view.ap?num=<%=dto.getNum()%>&p=<%=i%>"><%= i%></a></li>
  	<% }  else {%>
